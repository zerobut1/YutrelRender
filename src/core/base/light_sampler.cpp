    ...
    : m_renderer(renderer)
{
    ...
    {
        auto [view, buffer_id] = renderer.bindless_arena_buffer<Light::Handle>(light_handles.size());
...
LightSampler::Evaluation LightSampler::evaluate_hit(const Interaction& it, const Interaction& it_from, const SampledWavelengths& swl, Expr<float> time) const noexcept
{
    ...
    auto light_handles = renderer().light_handles();
    if (light_handles.empty()) [[unlikely]]
    {
    ...
    auto n                = static_cast<float>(light_handles.size());
...
LightSampler::Selection LightSampler::select(Expr<float> u, Expr<float> time) const noexcept
{
    ...
    auto n = static_cast<float>(renderer().light_handles().size());
...
LightSampler::Sample LightSampler::sample_selection(const Interaction& it_from, const Selection& sel, Expr<float2> u, const SampledWavelengths& swl, Expr<float> time) const noexcept
{
    ...
    if (renderer().environment() != nullptr)
    {
        if (renderer().light_handles().empty())
        {
    ...
    else if (!renderer().light_handles().empty())
    {
        sample = sample_light(it_from, sel, u, swl, time);
    }
    return sample;
...
{
    LUISA_ASSERT(!renderer().light_handles().empty(), "No finite lights in the scene.");
    auto handle       = renderer().buffer<Light::Handle>(m_light_buffer_id).read(sel.tag);
    auto light_sample = Light::Sample::zero(swl.dimension());
    renderer().lights().dispatch(handle.light_tag, [&](auto light) noexcept
    {
        auto closure = light->closure(swl, time);
        light_sample = closure->sample_li(handle.instance_id, it_from, u);
    });
...
{
    return Sample{.eval = s.eval, .shadow_ray = it_from.spawn_ray_to(s.p), .delta = s.delta};
}
