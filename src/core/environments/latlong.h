...
class LatLongEnvironment final : public Environment
{
public:
    class Instance;
...
public:
    LatLongEnvironment(const Texture* emission, float scale, float3x3 transform_to_world) noexcept
    ...
    [[nodiscard]] static Float2 direction_to_uv(Expr<float3> direction) noexcept;
    [[nodiscard]] static Float3 uv_to_direction(Expr<float2> uv) noexcept;
...
class LatLongEnvironment::Instance final : public Environment::Instance
{
private:
...
private:
    ...
    [[nodiscard]] static Float _directional_pdf(
        Expr<float> pdf_uv, Expr<float> theta) noexcept;

public:
    Instance(const Renderer& renderer, const LatLongEnvironment* environment,
...
class LatLongEnvironmentSpec final : public EnvironmentSpec
{
private:
...
public:
    LatLongEnvironmentSpec(TextureRef emission, float scale, float3x3 transform_to_world) noexcept
        : _emission{emission}, _scale{scale}, _transform_to_world{transform_to_world} {}
